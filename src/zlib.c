/* zlib.c - Zlib (RFC1950 / RFC1951) compression for PuTTY.
...
static int
lz77_hash (unsigned char *data)
{
  return (257 * data[0] + 263 * data[1] + 269 * data[2]) % HASHMAX;
}

static int
lz77_init (struct LZ77Context *ctx)
{
  struct LZ77InternalContext *st;
  int i;

  st = (struct LZ77InternalContext *) _gsti_xmalloc (sizeof (*st));
  if (!st)
    return 0;
...
static void
lz77_advance (struct LZ77InternalContext *st, unsigned char c, int hash)
{
  int off;
...
static void
lz77_compress (struct LZ77Context *ctx,
	       unsigned char *data, int len, int compress)
{
...
static void
outbits (struct Outbuf *out, unsigned long bits, int nbits)
{
  ...
  while (out->noutbits >= 8)
      ...
      if (out->outlen >= out->outsize)
	{
	  ...
	  out->outbuf = _gsti_xrealloc (out->outbuf, out->outsize);
	}
...
static void
zlib_literal (struct LZ77Context *_ectx, unsigned char c)
{
  struct Outbuf *out = (struct Outbuf *) _ectx->userdata;
...
static void
zlib_match (struct LZ77Context *_ectx, int distance, int len)
{
...
_gsti_compress_init (void)
{
  ...
  out = _gsti_xmalloc (sizeof (struct Outbuf));
  out->outbits = out->noutbits = 0;
  out->firstblock = 1;
  out->comp_disabled = FALSE;
  ectx.userdata = out;

  _gsti_log_info ("Initialised zlib (RFC1950) compression");
}
...
int
_gsti_compress_block (unsigned char *block, int len,
...
{
  struct zlib_table *tab = _gsti_xmalloc (sizeof (struct zlib_table));
  int pfxmask = (1 << pfxbits) - 1;
  int nbits, i, j, code;

  tab->table = _gsti_xmalloc ((1 << bits) * sizeof (struct zlib_tableentry));
  tab->mask = (1 << bits) - 1;
 ...
 */
static struct zlib_table *
zlib_mktable (unsigned char *lengths, int nlengths)
{
...
static int
zlib_freetable (struct zlib_table **ztab)
{
...
_gsti_decompress_init (void)
{
  ...
  dctx.bits = 0;
  dctx.nbits = 0;
  _gsti_log_info ("Initialised zlib (RFC1950) decompression");
}

int
zlib_huflookup (unsigned long *bitsp, int *nbitsp, struct zlib_table *tab)
{
  ...
  if (dctx.outlen >= dctx.outsize)
    {
      ...
      dctx.outblk = _gsti_xrealloc (dctx.outblk, dctx.outsize);
    }
...
int
_gsti_decompress_block (unsigned char *block, int len,
			unsigned char **outblock, int *outlen)
{
